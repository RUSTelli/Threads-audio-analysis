...
for language in DATASETS.keys():
    dataset = data_pipeline(classification_type="sentiment", language=language, is_multi_lang_model=False)
...
for language in DATASETS.keys():
    dataset = data_pipeline(classification_type="sentiment", language=language, is_multi_lang_model=True)
